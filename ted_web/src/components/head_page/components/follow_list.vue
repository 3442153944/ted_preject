  ...
  <div class="follow_list">
    <div class="content" v-if="follow_list.length>0">
        ...
        <div class="follow_list_item" v-for="(item,index) in follow_list" :key="index">
           <div class="avatar">
            <img :src="'http://localhost:8000/static/img/thumbnail/'+(item.avatar_path?item.avatar_path+'.png':'default_avatar.png')">
           </div>
           <div class="username" @click="to_other_user_center(item.target_user_id)">
            <span>{{item.username}}</span>
           </div>
        </div>
    </div>
    <div class="check_point" ref="check_point" style="display:block;width:1px;height:1px;opacity:0;pointer-events:none;"></div>
  </div>
</template>
...
async function get_follow_list(){
    try{
        const res=await fetch('http://localhost:8000/api/user/GetFollowList/',{
            method:'post',
            headers:{
